...
export default async function CurrentWeather({ weather }: CurrentweatherProps) {
  ...
  return (
    <section
      className={`w-[90%] h-2/5 flex rounded-xl justify-between mt-5 font-sans select-none  max-sm:h-72`}
      style={{ backgroundColor: colors.bgColor, color: colors.textColor }}
    >
      <div className="m-8 flex flex-col justify-between w-[50%] max-sm:w-full max-sm:m-4">
        <div className="flex w-full items-center justify-between">
          <div className="flex flex-row items-center">
            <LocationSvg color={colors.textColor} />
            <h2 className="text-xl capitalize max-sm:text-base">
              {weather?.city}
            </h2>
            <SignedIn>
              <SaveLocationButton
          ...
          <h2 className="font-light max-sm:text-sm">
            Dnes {await ConvertTime(weather?.current.dt, weather?.timezone)}
          </h2>
        </div>
        <div className="flex w-full flex-col items-center justify-between">
          <h2 className="mb-4 text-8xl max-sm:text-6xl">
        ...
        <div className="flex w-full justify-between max-sm:text-sm">
          <div className="flex flex-row items-center ">
