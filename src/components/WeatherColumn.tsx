...
export default async function WeatherColumn({
  ...
  return (
    <section className="flex h-full flex-col items-center font-sans w-[25%] max-sm:w-full ">
      <div className="flex w-full justify-around pt-9 pb-3 max-sm:tex-base max-sm:pb-0 max-sm:justify-evenly">
        <button>{"<"}</button>
      ...
      <div className="w-[80%] ">
        ...
        <div className="overflow-auto whitespace-nowrap pb-5">
          ...
          {weather.hourly.slice(1, 25).map((hourly, index) => (
            <HourlyWeather
              key={hourly.dt}
              hourly={weather.hourly[index + 1]}
              timezone={weather.timezone}
            />
          ))}
        </div>
        {weather.daily
          .slice(1, weather.daily.length + 1)
          .map(async (daily, index) => (
            <div
              key={index}
            ...
            >
              <div className="flex flex-col">
                <div className="w-20">
                  {index !== 0 ? await GetDayName(daily.dt) : "Tommorow"}
                </div>
                <div className="font-light text-gray-500">
                  {await GetDate(daily.dt)}
                </div>
              </div>
